  ...
  useEffect(() => {
    if (isAuthenticated && user?.agency && typeof user.agency === 'object' && user.agency._id) {
      const fetchUnreadCounts = async () => {
        try {
          const agencyId = (user.agency as { _id: string })._id;
          const messageResponse = await messageAPI.getUnreadCount(agencyId);
          const alertResponse = await alertAPI.getUnreadCount(agencyId);
          
          setUnreadMessages(messageResponse.count);
          setUnreadAlerts(alertResponse.count);
        } catch (error) {
